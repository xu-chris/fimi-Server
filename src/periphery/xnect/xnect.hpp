...
XNECT::XNECT(std::string configFile) : XNECT_implementation(configFile = "../../data/FullBodyTracker/") {
    ...
    if (m_NumChannels != -1) {
        CHECK(m_NumChannels == input_layer->channels())
                        << "FirstNet: Input layer should have same number of channels as first net, namely "
                        << m_NumChannels << ".";
    } else
...
void XNECT::processImg(cv::Mat &img) {
    ...
    m_NumCurrentPeople = postCNNFirstNet(part_affinity_heatmaps, peaks, nms_layer_scale_w, nms_layer_scale_h,
                                         imgOrigSize);
    ...
    input_layer->Reshape(m_NumCurrentPeople, m_NumFeatSecondNet + 3, m_NumJoints,
                         1); //  feature vector size 256 + u,v joint coordinate
    ...
    preCNNSecondNet(input_data, feature_data, m_FirstNet->blob_by_name(m_FeatureBlobName)->height(),
                    m_FirstNet->blob_by_name(m_FeatureBlobName)->width());
